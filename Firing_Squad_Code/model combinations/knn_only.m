...
for tr = 1:size(testData, 1)
    ...
    for direc = randperm(8)
        ...
        for t = times
            ...
            decodedHandPos = [decodedHandPos, decodedPos]; % Accumulate decoded positions
            trueDirections = [trueDirections; direc];
            % predictedDirections = [predictedDirections; predictedDir];
...
% Calculate and display RMSE and accuracy
...
%% Output and weight are hard coded as neuraldata
...
function [x,y,modelParameters]= positionEstimator(testData, modelParameters)
    ...
    % Classification is applicable within the time range
     ...
     % test_weight = testData;  % Directly use testData if it is prepared appropriately
...
function [output_lbl] = KNN_classifier(test_weight, train_weight, NN_num)
    % Ensure the weights are in correct format:
    % Assuming train_weight and test_weight are [features x samples]
    ...
    for i = 1:size(test_weight, 2)
        % Subtracting each test sample from all train samples
        % Transposing test sample to ensure [features x 1] dimension

        differences = bsxfun(@minus, train_weight, test_weight(:, i));
