...
%%%%%% 3. Most Discriminant Feature Analysis : PCA + LDA %%%%%%
    ...
    [output, weight] = MDF(neuraldata, directions, traininglen, lda_dimension);
...
function [output, weight] = MDF(neuraldata, directions, traininglen, lda_dimension)
    % Most Discriminant Feature Method using only LDA:
    % This function performs dimensionality reduction on neural data using Linear Discriminant Analysis (LDA)
    % to maximize class separability across different directions or classes. 
    ...
    % Output:
    %  output: The projected data after applying LDA for dimensionality reduction
    ...
    % Perform LDA
    [eigenvect, eigenval] = eig(pinv(scatter_within) * scatter_between);
    
    % Sort eigenvalues and eigenvectors in descending order
    [~, sortedComp] = sort(diag(eigenval), 'descend');
    eigenvect = eigenvect(:, sortedComp);
    
    % Select the lda_dimension dimensions that maximize class separability
    output = eigenvect(:, 1:lda_dimension);
