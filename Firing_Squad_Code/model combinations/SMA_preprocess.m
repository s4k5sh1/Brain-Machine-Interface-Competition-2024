...
RMSE_values = zeros(size(window_sizes));

% Loop over each window size
for win_idx = 1:length(window_sizes)
    win = window_sizes(win_idx);
    
    % Train Model using the specified window size for smoothing
    modelParameters = positionEstimatorTraining(trainingData, win);
...
figure;
plot(window_sizes, RMSE_values, 'b-o');
title('RMSE vs. SMA Window Size');
xlabel('Window Size');
ylabel('RMSE');
grid on;

% After calculating RMSE for each window size

% Find the index of the minimum RMSE
[minRMSE, optimalIndex] = min(RMSE_values);
optimalWindowSize = window_sizes(optimalIndex);

% Display the optimal window size and corresponding RMSE
fprintf('Optimal window size: %d, with RMSE: %.4f\n', optimalWindowSize, minRMSE);

% Train the model using the optimal window size
modelParameters = positionEstimatorTraining(trainingData, optimalWindowSize);
...
axis square;
grid on;
title(sprintf('True vs. Decoded Hand Positions Using Optimal Window Size: %d', optimalWindowSize));
...
for tr = 1:size(testData,1)
    for direc = randperm(8)  % random permutation of directions
        decodedHandPos = [];
        times = 320:20:size(testData(tr, direc).spikes, 2);
        ...
        plot(decodedHandPos(1, :), decodedHandPos(2, :), 'r');
        plot(testData(tr, direc).handPos(1, times), testData(tr, direc).handPos(2, times), 'b');
        legendInfo{end+1} = 'Decoded Position';
        legendInfo{end+1} = 'Actual Position';
    end
end

% Add legend to the plot
legend(legendInfo{1:2}); % To avoid duplicate legend entries

% Calculate final RMSE with optimal window size
finalRMSE = sqrt(meanSqError / n_predictions);
fprintf('Final RMSE with optimal window size: %.4f\n', finalRMSE);
...
function  modelParameters = positionEstimatorTraining(trainingData, win)
...
%%%%%% 1. Preprocess data  %%%%%%
trialFinal = process_and_smooth(trainingData, group, win);
...
function processed_data = process_and_smooth(trial, group, window_size)
...
%  window_size : size of the moving average window
    ...
    for i = 1:size(trial, 2)
        for j = 1:size(trial, 1)
            ...
            % Apply SMA smoothing
            sma_rates = zeros(size(spikes));
            for n = 1:neuron_num
                for t = window_size:size(spikes, 2)
                    sma_rates(n, t) = mean(spikes(n, t-window_size+1:t), 2);
                end
                % To handle the beginning where t < window_size, just average up to t
                for t = 1:window_size-1
                    sma_rates(n, t) = mean(spikes(n, 1:t), 2);
                end
            end

            % Store processed data in the output structure
            processed_data(j, i).rates = sma_rates / (group / 1000); % Adjust rates to spikes per second
        end
    end
end
