...
%%%%%% 3. Most Discriminant Feature Analysis : PCA + LDA %%%%%%
    ...
    [output, weight] = MDF(neuraldata, pca_dimension);
...
function [output, weight] = MDF(neuraldata, pca_dimension)
    ...
    % Output:
    %  output: The projected data after applying PCA for dimensionality reduction
    %  weight: The weights (principal components) used for projection

    % Perform PCA on the neural data
    [coeff, score, ~] = pca(neuraldata);

    % Select the first pca_dimension principal components
    weight = coeff(:, 1:pca_dimension);

    % Project the original data onto these components
    output = score(:, 1:pca_dimension);
