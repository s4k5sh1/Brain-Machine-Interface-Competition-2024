%Fjona Lutaj, Cristina Lopez Ruiz, Soranna Bacanu, Sakshi Singh
...
%%%%%% 4. SVM Classifier %%%%%%
    ...
    % Train the SVM model using PCA and LDA output features
    modelParameters.classify(count).svmModel = fitcecoc(weight', labels);
...
function [x,y,modelParameters, predicted]= positionEstimator(testData, modelParameters)
    ...
    %%%%%% 4. Use SVM to classify direction  %%%%%
    % Classification of movement direction using a SVM classifier. If the current 
    ...
    % Classification is applicable within the time range
     ...
     svmModel = modelParameters.classify(indexer).svmModel;
     [outLabel, score] = predict(svmModel, test_weight');
    else
